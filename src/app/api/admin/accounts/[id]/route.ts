...
import { adminApiGuard, updateInternalAccount } from "@/lib/internal/admin";
import { getInternalSession } from "@/lib/internal-auth";
import { validateInternalSession, ValidationError } from "@/lib/internal-db";

export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  const session = await validateInternalSession(await getInternalSession(), { roles: ["admin"] });
  ...
  try {
    await updateInternalAccount(session!, (await params).id, body as Record<string, unknown>);
    return apiOk();
  } catch (error) {

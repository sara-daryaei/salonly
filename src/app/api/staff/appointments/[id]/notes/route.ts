...
import { addStaffAppointmentNote, ForbiddenError, validateInternalSession, ValidationError } from "@/lib/internal-db";
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    await addStaffAppointmentNote({
      appointmentId: id,
      staffId: session.staffId,
      actorProfileId: session.profileId,
      note: String("note" in body ? body.note : ""),
      customerNote: Boolean("customerNote" in body ? body.customerNote : false),
    });
    return apiOk();
  } catch (error) {
